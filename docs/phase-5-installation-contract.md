...
- `setup_implemented: true` for the Core-only `0.0.7` catalog;
...
`packaging/installation-release-manifest.json` remains `ready: false` until exact installer and Core artifacts are published and clean-machine acceptance passes. Schema version 1 accepts `ready: true` only when every required artifact has its exact production URL, size, and SHA-256. The installer exposes a manifest-authorized `install` command, records durable install phases, activates the user service, and verifies the installed Core-only release. Browser generation remains outside `0.0.7`; doctor treats browser launch composition as not applicable for releases that do not require it.
...
- same-release repair, service-integrated uninstall, keep-data reinstall, purge acceptance, and the bounded `0.0.1` to `0.0.7` installer-only migration pass; general cross-version rollback remains a later-release feature;
