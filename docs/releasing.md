...
After publication, stable bootstrap promotion is a separate approved operator action. Cloudflare deployment configuration and credentials intentionally live outside this source repository. From the private operator workspace, run `.dispatch-cloudflare/promote <version>` only for the exact approved latest immutable Release. The promotion tool verifies the Release, tag, current `main` commit, source safety, canonical root `install.sh`, public edge bytes, and defensive headers before reporting success. A failed release is corrected by a new source commit and new tag; never overwrite an existing tag or silently substitute a different checkout.
