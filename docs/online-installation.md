...
Production installation remains fail-closed. `packaging/installation-release-manifest.json` has `ready: false`; the `install` command rejects that draft and accepts only a complete manifest plus the exact Core wheel it authorizes. User-service activation and setup for the current capability-free Handbook catalog are implemented. Reviewed release artifacts and clean-machine acceptance remain incomplete. Browser authority is intentionally deferred until a future approved plugin that requires it is selected.
