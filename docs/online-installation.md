...
The canonical bootstrap is the repository-root `install.sh`. Explicitly approved private operator tooling outside this repository fetches that exact file from an exact reviewed current-`main` commit, verifies that the commit is still current `main`, stages it transiently for Cloudflare, and verifies the public bytes and headers; no second tracked bootstrap implementation exists. The bootstrap resolves the selected channel at invocation time: stable selects the latest published immutable Release, while development selects the latest commit on `main`.
