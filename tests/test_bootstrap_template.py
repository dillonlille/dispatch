...
def test_bootstrap_template_is_fail_closed_and_shell_valid(tmp_path: Path) -> None:
    ...
    rendered = (
        content.replace("@PRODUCT_VERSION@", "0.0.3")
        .replace("@MANIFEST_URL@", "https://dispatch.dillonlille.com/releases/0.0.3/installation-release-manifest.json")
        .replace("@MANIFEST_SHA256@", "0" * 64)
        .replace("@INSTALLER_URL@", "https://dispatch.dillonlille.com/releases/0.0.3/dispatch_installer-0.1.2-py3-none-any.whl")
