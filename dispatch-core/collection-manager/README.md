...
`CollectionService.tick()` performs orphan cleanup, expired PID-less lease reconciliation, due-schedule enqueueing, and one supervised worker invocation. `CollectionService.run()` is a foreground, explicitly stoppable loop and starts no hidden threads. Installation as a persistent OS service remains installer-owned and deferred.
