...
from dispatch_core.paths import DispatchPaths, require_within
...
class PlaywrightRuntime:
    """Launch only an installer-authorized Chromium through Playwright."""
        ...
        try:
            ...
            with _playwright_driver_environment(profile, identity.control_executable):
                ...
                if spec is None or spec.origin is None:
                    raise BrowserManagerError("playwright_missing", "required Playwright package is not installed")
                if Path(spec.origin).resolve(strict=True) != installation.playwright_module:
                    raise BrowserManagerError("playwright_package_mismatch", "Playwright import origin is not approved")
                try:
                    import playwright
                ...
                if module_path != installation.playwright_module:
                    raise BrowserManagerError("playwright_package_mismatch", "loaded Playwright module is not approved")
                try:
                    from playwright._impl._driver import compute_driver_executable
                    from playwright.sync_api import sync_playwright
                except ImportError as exc:
                    raise BrowserManagerError("playwright_missing", "required Playwright package is incomplete") from exc
                driver_executable, driver_cli = (Path(value).resolve(strict=True) for value in compute_driver_executable())
                if driver_executable != identity.control_executable:
                    raise BrowserManagerError("playwright_driver_mismatch", "Playwright control executable is not approved")
                if driver_cli != installation.playwright_driver_cli:
                    raise BrowserManagerError("playwright_driver_mismatch", "Playwright driver CLI is not approved")
